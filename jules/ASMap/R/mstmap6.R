...
mstmap.cross <- function(object, chr, id = "Genotype", bychr = TRUE, suffix = "numeric",
                   anchor = FALSE, ril.gen = 0, dist.fun = "kosambi", objective.fun = "COUNT",
                   p.value=1e-6, noMap.dist = 15.0, noMap.size = 0, miss.thresh = 1.0,
                   mv.est = FALSE, detectBadData = FALSE, trace = FALSE, ...) {
    ...
    if(!inherits(object, c("bc","dh","riself","f2")))
        stop("Cross object must inherit from one of the classes \"bc\", \"dh\",\"riself\",\"f2\". See ?mstmap.cross for more details.")
    ...
    if(class(object)[1] %in% c("bc","dh","riself")){
        pop.type <- "DH"
        if(ril.gen > 0)
            warning("Class of population is ", class(object)[1], " ... ignoring \"ril.gen\"")
        allele <- c("A","B","")
    }
    if(class(object)[1] %in% "f2"){
        if(ril.gen == 0)
            stop("Class of population is f2 .. \"ril.gen\" is required to be greater than 0")
        pop.type <- paste("RIL", ril.gen, sep = "")
        allele <- c("A","X","B")
    }
    ...
    param <- list("population_type"=pop.type,
                ...
                "estimation_before_clustering"=as.integer(mv.est),
                "detect_bad_data"=as.integer(detectBadData),
                "objective_function"=objective.fun,
                trace=as.integer(trace))
    omit.list <- list()
    for(i in 1:length(lmat)){
        map <- .Call("mst", param, as.data.frame(lmat[[i]], stringsAsFactors = FALSE))
        chrw <- nm[i]
        nmm <- names(object$geno[[chrw]]$map)
        if(anchor)
            map <- lapply(map, function(el, nmm){
                  nmms <- nmm[nmm %in% names(el)]
                  lens <- 1:length(nmms)
                  apos <- pmatch(nmms, names(el))
                  rpos <- pmatch(nmms, names(rev(el)))
...
mstmap.data.frame <- function(object, pop.type= "DH", dist.fun = "kosambi",
                   ...
                   miss.thresh=1.0, mv.est=FALSE, detectBadData=FALSE, trace=FALSE, as.cross = TRUE, ...) {
    if(trace) trace <- "MSToutput.txt"
    if (is.character(trace)) {
        ftrace <- file(trace, "w")
        sink(trace, type = "output", append = FALSE)
    ...
    if(any(!sapply(object, is.character)))
        stop("Some of the columns of the marker data.frame are not of type \"character\"")
    if(!(pop.type %in% c("DH","RIL")))
        stop("Population type needs to be \"DH\" or \"RILx\" (see ?mstmap.data.frame).")
    if(length(grep("RIL", pop.type))){
        rnum <- substring(pop.type, 4, nchar(pop.type))
        if(!(as.numeric(rnum) %in% 1:100))
            stop("Illogical RIL \"pop.type\" argument.")
    }
    if(!(dist.fun %in% c("haldane","kosambi")))
    ...
    param <- list("population_type"=pop.type,
                ...
                "objective_function"=objective.fun,
                trace=as.integer(trace))
    map <- .Call("mst", param, object)
    ordm <- pmatch(rownames(object), unlist(sapply(map, names)))
    if(any(is.na(ordm)))
    ...
    if(as.cross){
        ...
        cons <- cons[1:length(allow.list)]
        wh <- allow.list %in% alleles
        cons <- cons[wh]
        for(i in 1:length(alleles))
            object[object == alleles[i]] <- cons[i]
        datm <- lapply(split(as.data.frame(object), spl), t)
        ...
        class(co) <- c(cclass,"cross")
        object <- co
    } else {
        do <- list()
        chrv <- rep(chn, times = sapply(map, length))
        do$geno <- cbind.data.frame(markers = unlist(sapply(map, names)), chr = chrv, dist = unlist(map), object)
        object <- do
    }
    if(exists("omit.list"))
...
pullCross <- function(object, chr, type = c("co.located","seg.distortion","missing"), pars = NULL, replace = FALSE, ...){
    ...
    type <- match.arg(type)
    if(!(class(object)[1] %in% c("bc","dh","riself","f2")))
        stop("Pulling of markers is not supported for this population type.")
    oldObject <- NULL
    if (!missing(chr)) {
        oldObject <- subset(object, paste("-", chr, sep =""))
    ...
    if(type %in% c("seg.distortion","missing")){
        ...
        tab <- cbind.data.frame(mark = markn[whm], tab[whm,])
    }
    rownames(tab) <- NULL
    if(dim(tab)[1] != 0){
...
## fastGroup <- function(object, chr, min.lod = 3, max.rf = 0.25){
##     if(missing(chr))
##         stop("chr is a required argument (see ?fastGroup)")
##     push.object <- subset(object, chr)
##     oe <- subset(object, paste("-", chr, sep =""))
##     oe$geno <- lapply(oe$geno, function(el){
##         hm <- floor(el$map[length(el$map)]/10)
##         if(hm != 0){
##             whp <- seq(el$map[1], el$map[length(el$map)], length.out = 2 + hm)
...
combineMap <- function(..., id = "Genotype", keep.all = TRUE){
    ...
    maplb <- lapply(mapl, function(el){
        ...
        dimnames(mapb)[[2]] <- paste(chrs, markernames(el), as.character(mdist), sep = ";")
    ...
    mapm <- mapm[mixedorder(mapm[[id]]),]
    nams <- mapm[[id]]
    mapm <- mapm[,!(names(mapm) %in% id)]
    spl.m <- strsplit(names(mapm), ";")
    ch <- sapply(spl.m, "[", 1)
    mapm <- as.data.frame(t(mapm))
    mapf <- list()
    mapf$geno <- lapply(split(mapm, ch), function(el, nams){
        temp <- list()
    ...
    print(names(mapf$geno))
    mapf$pheno[[id]] <- factor(nams)
...
statMark <- function(cross, chr, stat.type = c("marker","interval"), map.function = "kosambi"){
    if (!any(class(cross) == "cross"))
        stop("Input should have class \"cross\".")
    if(!(class(cross)[1] %in% c("bc","dh","riself","f2")))
        stop("This function not suitable for this cross stat.type.")
    if (!missing(chr))
        ...
        mrk$marker$dxo <- unlist(lapply(cross$geno, function(el) {
            if(ncol(el$data) < 3) sxo <- rep(0, ncol(el$data))
                ...
                sxo <- sapply(3:ncol(el$data), function(i, el) {
                    left <-  abs(el$data[,i-2] - el$data[,i - 1]) > 0
                    right <- abs(el$data[,i] - el$data[,i - 1]) > 0
                    sm <- left + right
                    sm[abs(sm) != 2] <- 0
                    sm[abs(sm) == 2] <- 1
                    sum(sm, na.rm = TRUE)
    ...
    if("interval" %in% stat.type){
        ...
        mrk$interval$recomb <- unlist(lapply(icross$geno, function(el){
            sapply(2:ncol(el$data), function(i, el)
                  sum(abs(el$data[,i] - el$data[,i - 1]), na.rm = TRUE), el)
        }))
...
profileMark <- function(cross, chr, stat.type = "marker", use.dist = TRUE, map.function = "kosambi",
    ...
    if (!any(class(cross) == "cross"))
        stop("Input should have class \"cross\".")
    if(!(class(cross)[1] %in% c("bc","dh","riself","f2")))
        stop("This function not suitable for this cross type.")
    if (!missing(chr))
    ...
    if(!all(stat.type %in% c(mtypes, stypes)))
        stop("Value for type argument does not match allowable names, see ?profileMark.")
    ...
    stat <- statMark(cross, chr = names(nmar(cross)), stat.type = call.type, map.function)
    nm <- nmar(cross)
    if(use.dist) {
...
statGen <- function(cross, chr, bychr = TRUE, stat.type = c("xo","dxo","miss"), id = "Genotype"){
    if (!any(class(cross) == "cross"))
        stop("Input should have class \"cross\".")
    if(!(class(cross)[1] %in% c("bc","dh","riself","f2")))
        stop("This function not suitable for this cross type. ?see countGen")
    if (!missing(chr))
        cross <- subset(cross, chr = chr)
    nch <- nchr(cross)
    nm <- nmar(cross)
...
profileGen <- function(cross, chr, bychr = TRUE, stat.type = c("xo","dxo","miss"), id = "Genotype", xo.lambda = NULL, ...){
    ...
    if (!missing(chr))
        cross <- subset(cross, chr = chr)
     nm <- nmar(cross)
     cnt <- statGen(cross, chr = names(nm), bychr, stat.type, id)
    ...
    if(bychr){
     ...
     cnt <- unlist(cnt)
     form <- cnt ~ noind | stat.type
     grp <- rep(1, length(noind))
     cntn <- cnt; noindn <- noind
     plot.xo <- FALSE
     if(!is.null(xo.lambda) & any(xo.lambda)){
        inds <- (1:length(phv))[rep(xo.lambda,no.splits)]
        grp[inds] <- 2
        cntn[-inds] <- noindn[-inds] <- phv[-inds] <- NA
        ylims <- lapply(split(cnt, stat.type), function(el) c(0, max(el)[1] + 0.15*max(el)[1]))
        plot.xo <- TRUE
    } else ylims <- lapply(split(cnt, stat.type), function(el) c(0, max(el)[1]))
